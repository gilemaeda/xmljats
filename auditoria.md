...
Gerada por `python ops/auditoria.py` (app 0.27.1). Cada número vem de uma medição desta rodada: os PDFs foram reprocessados, os XML gerados e validados no packtools, e o site exercitado ponta a ponta.
...
- ok — conta nova não recebe janela de novidades (nada mudou para ela)
- ok — novidades: janela para quem ainda não viu a versão
- ok — cliente não vê novidade do painel administrativo
...
| Lote de entrega: até 5 artigos da mesma revista/número num só pacote, com um aviso | pronto | ops/test_lotes.py |
