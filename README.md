...
Segue a seção **Entrega de Pacote XML para Publicação** da documentação SPS 1.10 (o guia de dez/2024 em PDF foi incorporado a ela). A SciELO não publica API de depósito: o que existe é um FTP e um e-mail obrigatório — e o sistema faz os dois, na tela **Entrega à SciELO** de cada documento.

1. Chegar a "Pronto para entrega" (zero bloqueantes, validador oficial sem erro).
2. A tela confere o pacote contra a SPS: é `.zip` (`.rar` não é aceito) com **uma pasta de mesmo nome dentro** (`ISSN-acrônimo-volume-número`, mais o lote em publicação contínua, ex.: `2179-8966-rdp-17-03-0126`), os nomes não têm ponto extra, underline nem acento, o XML é válido no DTD e no Schematron, todos os arquivos citados pelo XML estão dentro e o relatório de validação (`xpm.html`) vai dentro da pasta.
3. Depósito no FTP: conta de prestador (com atestado) deposita em `Entrega` e `Correcao` na raiz; conta da própria revista no FTP da SciELO deposita em `<acrônimo>/Entrega`. O tipo de conta e a sigla da coleção ficam em Configurações. Em publicação contínua, cada depósito é um lote numerado em sequência por volume/número (`lotes.json`), e o número vai no nome da pasta e no e-mail. As credenciais são pedidas a `publicacao@scielo.org` e ficam em **Configurações**, gravadas só em `XMLJATS_DATA/config.json` (a senha nunca aparece inteira em tela).
4. Aviso por e-mail: depositar sozinho **não garante publicação**. Ao depositar daqui, o aviso para `publicacao@scielo.org` já fica como rascunho no correio, com cópia à equipe editorial da revista, no formato da SPS 1.10: título `Entrega | rdp v17n3 Lote 0126 - BR` (publicação contínua) ou `Entrega | rdp v17n3 2026 - BR` (regular; `Retrô Entrega` para volume de dois anos atrás ou mais) e corpo começando por "Informo que o .zip com a marcação XML do periódico …, foi disponibilizado no FTP. - Total de XMLs = 1."

O que entra no pacote: XML em SciELO PS (e `sub-article` por tradução), PDF por idioma, imagens em `.tiff`/`.jpg`/`.png`, **tabelas, fórmulas e equações codificadas em MathML ou LaTeX** (imagem não é aceita), relatório de validação (`xpm.html`, um por pacote), material suplementar e vídeos em `.mp4`. Em publicação contínua, o `article-id` do tipo `other` tem exatamente 5 dígitos e o título da seção precisa ser idêntico no PDF, na planilha de "Other" e no XML.
...
- Painel: documentos com filtros por revista, etapa e situação; a etapa do artigo no fluxo SciELO (Recebido → Em revisão → Pronto para entrega → Entregue à SciELO → Entrega confirmada → Pré-QA → QA → Correção pedida pela SciELO → QA finalizado → Publicado, com os nomes que a SciELO usa no título dos e-mails) é anotada à mão, com histórico de quem mudou e quando.
...
- **Conferência externa do XML (05/09/2026).** O Murillo mandou um XML nosso para uma análise independente. Dos oito pontos levantados, três eram defeito nosso e foram corrigidos: `(eds.)`/`(org.)` na abertura da referência viravam autor em vez de editor; a data de acesso saía sozinha quando o artigo trazia "Disponível em:" sem endereço; e o rodapé da revista ("Recebido: … Aceito: …") grudava na última referência. Um quarto ponto virou aviso: referência sem chamada no texto (R03). Um ponto não procedia: o "Uma outa linha" é erro do próprio PDF do artigo, não da extração. O `page-count`, também apontado, foi primeiro rejeitado (nenhum dos cinco XML publicados pela SciELO que usamos de gabarito o traz) e depois confirmado pela documentação da SPS; está na rodada seguinte.
...
- **Lista de documentos**: coluna com a data da última abertura (com "há quanto tempo" e, para o administrador, quem abriu) e seletor de ordenação: atualizado, **aberto mais recente**, enviado mais recente, mais antigo, título, revista, com mais bloqueantes e etapa no fluxo. A ordem convive com os filtros e volta marcada na tela.
- **Auditoria de 06/09/2026 (app 0.20.0), antes da fase comercial.** Furos fechados: o envio e o salvar-e-validar rodavam dentro do event loop e travavam o site inteiro enquanto um artigo era processado (agora vão para a thread pool); XML com bloqueante pendente sai marcado como rascunho num comentário logo abaixo da declaração, e os botões dizem "(rascunho)"; o resultado leva ao campo exato ("ir para o campo"); freio de tentativas no login (10 falhas por IP ou por e-mail em 15 minutos) e no registro (5 contas por IP em 15 minutos); tempo de processamento de cada artigo medido e mostrado no painel administrativo; e-mail da equipe editorial no cadastro da revista, em cópia no aviso de depósito; textos da ajuda e da tela inicial que ainda diziam "só PDF", "CRediT não é gerado" e "equação como imagem" corrigidos; dois testes que caíam por causa do console cp1252 do Windows. **SPS 1.10 passou a ser a versão padrão** e o Schematron 1.10 do packtools foi ligado (o packtools 4.16 só habilita 1.8 e 1.9 por padrão, então escolher 1.10 gerava XML sem validação de estilo e que nunca ficava pronto). **O pacote e o aviso de entrega seguem a SPS 1.10**, cuja seção de entrega absorveu o guia de dez/2024: pasta com o nome do pacote dentro do .zip, relatório `xpm.html`, lote numerado por volume/número em publicação contínua, título e corpo do e-mail no formato fixo, tipo de conta do FTP (prestador ou revista) e sigla da coleção. Etapas ganharam "Entrega confirmada" e "Correção pedida pela SciELO", os status dos e-mails dela. PDF escaneado (sem camada de texto) passava em silêncio como "título não identificado"; agora é detectado e vira bloqueante com explicação (D01), e OCR fica na lista do que falta. O painel mostra correções à mão por artigo (a taxa de erro do motor, medida pelo que o revisor mudou), e a tela de entrega traz o roteiro para a revista depositar sozinha.
- **Novidades e notificações (0.21.0).** As notas de cada versão ficam em `app/novidades.py`, com o público de cada item (todos, operador ou administrador). Ao entrar depois de uma atualização, uma janela mostra o que mudou para aquele perfil; o sino "Novidades" no menu leva à página com o histórico e conta o que ainda não foi visto. Conta nova nasce com a versão atual (nada é novidade para quem chegou agora); conta de antes do recurso usa a data do último acesso. O que é do painel administrativo nunca chega ao HTML do cliente: a filtragem é no servidor. `VERSAO_APP` e a primeira versão das novidades têm de ser iguais, senão o app não sobe. Cobertura em `ops/test_novidades.py`.
- **Fila de processamento (0.22.0).** O envio aceita vários arquivos (até 20, `XMLJATS_MAX_LOTE`); cada um vira um documento com estado `na_fila` → `processando` → `concluido`/`erro` no `config.json`, e um trabalhador em thread (`app/fila.py`, `XMLJATS_TRABALHADORES`, padrão 1) processa um por vez. A lista mostra o estado e atualiza sozinha; `/doc/{id}` vira uma página de espera com `/doc/{id}/estado.json` até o resultado existir; erro fica visível com o motivo e um "tentar de novo". A fila sobrevive a reinício (o que estava pendente volta para ela). Um arquivo só continua síncrono. Cobertura em `ops/test_fila.py`.
- **OCR de PDF escaneado (0.23.0).** Página sem camada de texto passa pelo Tesseract por dentro do PyMuPDF (`poc/extrator/ocr.py`, português + inglês, 300 dpi); o texto entra no mesmo fluxo, com aviso D02 e proveniência "OCR", e o visualizador ganha a camada de texto. PDF normal não passa pelo OCR. O container instala `tesseract-ocr-por` e `tesseract-ocr-eng` e aponta `TESSDATA_PREFIX`; na máquina de desenvolvimento serve a pasta do instalador do Tesseract ou `%LOCALAPPDATA%\xmljats\tessdata`. Sem dados de idioma, o PDF escaneado continua sendo avisado (D01) e `/saude` mostra `ocr: false`. Cobertura em `ops/test_ocr.py`.
- Falta (medido contra o plano, 06/09/2026): lote de entrega com vários artigos num só .zip (a fila processa em lote, a entrega ainda é por artigo); integração com o OJS; validador público sem conta (decisão dos sócios); modelo DOCX distribuível com estilos próprios e tabela de metadados; equipe da revista compartilhando os mesmos documentos (hoje cada conta vê só o que enviou, e o operador vê tudo); captcha no registro (Turnstile, precisa de chaves da Cloudflare); custo por artigo com a revisão humana medida (o tempo de máquina já é). Não há modelo de linguagem no pipeline: todo preenchimento automático é por regra, medido contra gabaritos.
