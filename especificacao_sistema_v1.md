...
Estado em 05/09/2026, fim do dia (app 0.6.0): figuras no pacote (TIFF, `fig`/`graphic`, `xref`); notas de rodapé com `xref` no corpo; `element-citation` completo por heurística ABNT/APA (69 de 76 campos iguais ao XML oficial da SciELO no artigo da Direito e Práxis); interface em modo claro e escuro com contraste medido; contas com login e papéis (telas 4 e 5 da seção 5: conta, painel, cadastro de revistas editável) e etapas do artigo no fluxo SciELO anotadas no painel. Nenhum dos seis PDFs de amostra tem tabela ou equação, por isso `table-wrap` e `disp-formula` ficaram de fora até haver um artigo real para testar (regra da casa: nada entra sem ser testado em PDF de verdade). O caminho DOCX aguarda os arquivos da ANAMORPHOSIS.
